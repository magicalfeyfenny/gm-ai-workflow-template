...
ADJUDICATION_RESULT_SCHEMA = "adversarial-adjudication-result:v5"
...
def validate_adjudication_result(
    ...
    try:
        ...
        disposition_fields = ("finding_id", "disposition", "basis")
        for index, value in enumerate(raw_dispositions):
            subject = f"adjudication disposition {index}"
            disposition = review._mapping(value, subject, structured_output=True)
            review._keys(
                disposition,
                disposition_fields,
                disposition_fields,
